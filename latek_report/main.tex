...
\definecolor{utbmblue}{HTML}{1F7EC2}     % Bleu de marque UTBM (logo)
\definecolor{utbmdark}{HTML}{17597F}     % Bleu UTBM foncé
\definecolor{primary}{gray}{0}           % Noir (titres)
\definecolor{accent}{HTML}{1F7EC2}       % Accent = bleu UTBM
...
\definecolor{tablerule}{gray}{0.2}       % Filets de tableau (gris foncé pro)
\definecolor{headerbg}{HTML}{EAF2F9}     % Bandeau d'en-tête de tableau (bleu très clair)
...
% Format des sections (avec filet fin sous le titre)
\titleformat{\section}
  {\color{primary}\bfseries\large}
  {\color{accent}\thesection}
  {0.8em}
  {}
  [{\color{utbmblue!35}\titlerule[0.8pt]}]
...
% --- Tableaux (style « papier de recherche » : booktabs) ---
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{multirow}
\usepackage{array}
...
% Filets booktabs : épaisseurs nettes et espacement aéré
\setlength{\heavyrulewidth}{1.1pt}   % \toprule / \bottomrule
\setlength{\lightrulewidth}{0.5pt}   % \midrule
\setlength{\cmidrulewidth}{0.5pt}
\setlength{\aboverulesep}{2.5pt}
\setlength{\belowrulesep}{2.5pt}

% En-tête de tableau : bandeau bleu très clair (sobre) + texte gras
\usepackage{colortbl}
\newcommand{\tableheadcolor}{\rowcolor{headerbg}}
...
% Boîte "Hypothèse / Observation" : callout à bandeau-titre bleu
\newtcolorbox{hypothesebox}[1][]{
  enhanced, breakable,
  colback=utbmblue!4, colframe=utbmblue,
  boxrule=0.6pt, leftrule=2.5pt, arc=2pt,
  coltitle=white, colbacktitle=utbmblue,
  fonttitle=\bfseries\small, title={#1},
  fontupper=\small,
  left=7pt, right=7pt, top=5pt, bottom=5pt
}

% Boîte "Décision / À retenir" : barre latérale épaisse (encadré conclusion)
\newtcolorbox{decisionbox}{
  enhanced, breakable,
  colback=gray!4, colframe=utbmdark,
...
\includegraphics[width=5.6cm]{figures/utbm.png}\\[0.9cm]

{\large\color{utbmdark}\textbf{UNIVERSITÉ DE TECHNOLOGIE DE BELFORT-MONTBÉLIARD}}\\[0.35em]
{\normalsize\color{darktext}Projet PF22 — Apprentissage profond}\\[2.2cm]

{\color{utbmblue}\rule{\linewidth}{2pt}}\\[0.18em]
{\color{midgray}\rule{\linewidth}{0.5pt}}\\[1.1em]

{\large\color{darktext}\textbf{RAPPORT DE PROJET}}\\[0.9em]
{\Huge\bfseries\color{primary}Réduction des Coûts d'Apprentissage}\\[0.45em]
{\Huge\bfseries\color{utbmblue}par Approche Multi-Fidélité}\\[1.1em]

{\color{midgray}\rule{\linewidth}{0.5pt}}\\[0.18em]
{\color{utbmblue}\rule{\linewidth}{2pt}}\\[1.7cm]
...
\begin{tabular}{r@{\hskip 0.6em}l}
{\color{utbmdark}\textbf{Auteur}}   & Ivann Vasic \\
{\color{utbmdark}\textbf{Période}}  & Mars -- Juin 2026 \\
{\color{utbmdark}\textbf{Cadre}}    & Projet PF22 --- UTBM \\
\end{tabular}

\vfill
{\color{midgray}\small Belfort-Montbéliard \quad\textbullet\quad 2026}
\end{center}
...
La figure~\ref{fig:dataset_samples} illustre, pour chaque dataset, une même image en Haute Fidélité et sa version dégradée Basse Fidélité telle que produite par le pipeline du projet.
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{c c c}
     & \textbf{Haute Fidélité (HF)} & \textbf{Basse Fidélité (BF)} \\
    \rotatebox{90}{\scriptsize\ \ Animals-10} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Animals-10_HF.png} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Animals-10_BF.png} \\
    \rotatebox{90}{\scriptsize\ \ Imagewoof} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Imagewoof_HF.png} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Imagewoof_BF.png} \\
    \rotatebox{90}{\scriptsize\ \ \ \ Intel} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Intel_HF.png} &
      \includegraphics[width=0.34\linewidth]{figures/sample_Intel_BF.png} \\
  \end{tabular}
  \caption{Une même image en Haute Fidélité (gauche) et après dégradation Basse
  Fidélité (droite : sous-échantillonnage 64~px + bruit $\sigma=0{,}15$ + JPEG $q60$),
  pour les trois datasets. La dégradation est produite par le pipeline canonique du
  projet, identique à celui appliqué à l'entraînement.}
...
\section{Définition de la métrique de coût}\label{sec:cout}
...
Les valeurs chiffrées détaillées figurent dans le \textbf{tableau de synthèse global} (§\ref{sec:synthese_strat}), qui rassemble toutes les approches sur les trois datasets. La comparaison se concentre ici sur l'écart à la borne supérieure, la \textbf{baseline Mixte (BL 3)}.
...
Les valeurs chiffrées détaillées figurent dans le \textbf{tableau de synthèse global} (§\ref{sec:synthese_strat}). La comparaison ci-dessous situe la Stratégie 2 par rapport à la \textbf{baseline Mixte (BL 3)} et à la \textbf{Stratégie 1}.
...
\subsection{Comparaison globale des stratégies}\label{sec:synthese_strat}
