...
\begin{figure}[htbp]
  ...
  \caption{Gains de précision Test HF apportés par l'optimisation Optuna (version optimisée $-$ version de base), par stratégie et par dataset.}
...
\begin{itemize}
  ...
  \item \textbf{Meilleures configurations après optimisation} : Animals-10 — S1+Optuna (78,1\,\%) ; Imagewoof — S5(EWC)+Optuna et S3+Optuna ($\approx$53 et 52,8\,\%) ; Intel — S1+Optuna et S3+Optuna ($\approx$86\,\%).
\end{itemize}
...
Chaque modèle entraîné (les 7 modèles : 3 baselines + 4 stratégies) est évalué sur des versions du jeu de test \textbf{progressivement dégradées}, selon deux balayages indépendants :
...
\begin{figure}[htbp]
  ...
  \caption{Robustesse sur \textbf{Animals-10} : précision en fonction de la perte de résolution (gauche) et du bruit gaussien (droite), pour les 7 modèles.}
...
\begin{figure}[htbp]
  ...
  \caption{Précision Test HF (moyenne $\pm$ écart-type) de tous les modèles, sur les trois datasets.}
