...
Le jeu de données doit être suffisamment difficile pour que la dégradation visuelle impacte mesurément les performances du réseau. Cinq candidats ont été évalués :
...
\vspace{0.8em}
\begin{decisionbox}
  \textbf{\color{primary}Choix retenu : Animals-10 \emph{et} Imagewoof (double validation)} \\[3pt]
  Afin de garantir que les conclusions tirées sur les stratégies multi-fidélité ne sont pas un artefact propre à un dataset particulier, \textbf{deux jeux de données ont été retenus} et le protocole expérimental complet est exécuté sur chacun :
  \begin{itemize}
    ...
    \item \textbf{Imagewoof} (12 954 images, 10 races de chiens) — dataset de \textbf{validation croisée}. Sous-ensemble d'ImageNet plus difficile (classification \textit{fine-grained} de classes visuellement très proches) et nettement plus petit en volume, il met les stratégies sous pression. Une stratégie qui reste la meilleure sur les deux datasets peut alors être considérée comme \textbf{robuste} et non liée aux spécificités d'un seul jeu de données.
  \end{itemize}
  Les deux datasets suivent strictement le même pipeline (répartition test 10\,\% / train 90\,\%, train découpé en 10\,\% HF + 90\,\% BF, même dégradation, ResNet-18 \textit{from scratch}, 20 époques, AMP FP16).
\end{decisionbox}
...
Les trois modèles de référence ont été entraînés avec succès sur 20 époques, sur chacun des deux datasets retenus. Les résultats obtenus, résumés dans le tableau ci-dessous, mettent en évidence la dynamique du compromis coût/performance.
...
\begin{tabularx}{\linewidth}{L{3cm} C{2.2cm} C{2cm} C{2cm} C{2cm} C{2.4cm}}
  ...
  \tablerowodd
  BL 1 (HF) & 179 800 & 24,87 \% & 19,04 \% & 21,95 \% & 0,39 min \\
  BL 2 (BF) & 162 520 & 43,73 \% & 44,52 \% & 44,12 \% & 1,00 min \\
  \tablerowodd
  BL 3 (Mixte) & 342 320 & 47,19 \% & 47,24 \% & 47,21 \% & 1,36 min \\
  \bottomrule
\end{tabularx}
\vspace{0.8em}
...
\begin{hypothesebox}[Vérification sur Imagewoof]
  ...
  \begin{itemize}
    \item \textbf{Domain Shift confirmé} : la BL 1 passe de 24,87\,\% (HF) à 19,04\,\% (BF), soit une chute de $-5,8$ points cohérente avec celle observée sur Animals-10.
    \item \textbf{Paradoxe de la quantité confirmé} : la BL 2 (162 520 CA) bat la BL 1 (179 800 CA) sur les trois tests, y compris HF (43,73\,\% vs 24,87\,\%), avec un budget inférieur.
    \item \textbf{Borne supérieure validée} : la BL 3 atteint 47,19\,\% sur HF pour un coût double de BL 1.
  \end{itemize}
  La \textbf{hiérarchie BL 3 $>$ BL 2 $>$ BL 1} est strictement conservée d'un dataset à l'autre, ce qui valide la robustesse des conclusions sur le comportement des baselines indépendamment du dataset utilisé.
\end{hypothesebox}
...
Les résultats finaux de l'Étape 1 sont résumés ci-dessous pour les deux datasets.
...
\begin{tabularx}{\linewidth}{L{5.3cm} C{2.4cm} C{2.4cm} C{2.4cm} C{2.4cm}}
  ...
  \midrule
  \multicolumn{5}{l}{\textit{\textbf{Animals-10}}} \\
  \tablerowodd
  Stratégie 1 (BF $\rightarrow$ HF) & 78,84\% & 57,65\% & 68,25\% & 400\,230 \\
  \midrule
  \multicolumn{5}{l}{\textit{\textbf{Imagewoof}}} \\
  \tablerowodd
  Stratégie 1 (BF $\rightarrow$ HF) & 53,68\% & 44,82\% & 49,25\% & 153\,160 \\
  \bottomrule
\end{tabularx}

\vspace{0.8em}
\begin{hypothesebox}[Lecture rapide des courbes d'entraînement (Animals-10)]
...
\begin{tabularx}{\linewidth}{L{4.5cm} C{2.2cm} C{2.2cm} C{2.2cm} C{2.5cm}}
  ...
  \multicolumn{5}{l}{\textit{\textbf{Imagewoof}}} \\
  \tablerowodd
  BL 1 (HF) & 24,87\% & 19,04\% & 21,95\% & 179\,800 \\
  BL 2 (BF) & 43,73\% & 44,52\% & 44,12\% & 162\,520 \\
  \tablerowodd
  BL 3 (Mixte) & 47,19\% & 47,24\% & 47,21\% & 342\,320 \\
  \textbf{Stratégie 1 (BF $\rightarrow$ HF)} & \textbf{53,68\%} & \textbf{44,82\%} & \textbf{49,25\%} & \textbf{153\,160} \\
  \bottomrule
\end{tabularx}
...
\begin{hypothesebox}[Vérification sur Imagewoof]
  Sur Imagewoof, la Stratégie 1 atteint \textbf{53,68\,\%} en Test HF, soit \textbf{le meilleur score HF parmi les quatre modèles testés} (BL 3 = 47,19\,\%, BL 2 = 43,73\,\%, BL 1 = 24,87\,\%). L'avantage de la stratégie séquentielle est même plus marqué que sur Animals-10 :
  \begin{itemize}
    \item \textbf{Vs BL 3 (Mixte)} : $+6,49$ points HF pour un coût divisé par $\approx 2,2$ (153\,160 vs 342\,320 CA), confirmation directe de l'efficacité de la séparation BF $\rightarrow$ HF.
    \item \textbf{Rétention BF satisfaisante} : 44,82\,\% sur BF, comparable à BL 2 (44,52\,\%) ; l'écart HF/BF se resserre légèrement par rapport à Animals-10, ce qui suggère qu'un problème \textit{fine-grained} à faible volume conserve une plus forte dépendance aux représentations basses fréquences.
  \end{itemize}
  La hiérarchie \textbf{Stratégie 1 $>$ BL 3 $>$ BL 2 $>$ BL 1} est strictement identique sur les deux datasets, ce qui confirme que la dominance du transfer learning séquentiel n'est pas un artefact d'Animals-10 et constitue bien une propriété robuste de la méthode.
\end{hypothesebox}
...
Les résultats finaux de l'Étape 2 sont résumés ci-dessous pour les deux datasets.
...
\begin{tabularx}{\linewidth}{L{5.3cm} C{2.4cm} C{2.4cm} C{2.4cm} C{2.4cm}}
  ...
  \multicolumn{5}{l}{\textit{\textbf{Imagewoof}}} \\
  \tablerowodd
  Stratégie 2 (Co-training pondéré) & 39,20\% & 39,81\% & 39,50\% & 183\,040 \\
  \bottomrule
\end{tabularx}
...
\begin{tabularx}{\linewidth}{L{4.5cm} C{2.2cm} C{2.2cm} C{2.2cm} C{2.5cm}}
  ...
  Stratégie 1 (BF $\rightarrow$ HF) & 53,68\% & 44,82\% & 49,25\% & 153\,160 \\
  \tablerowodd
  \textbf{Stratégie 2 (Co-training pondéré)} & \textbf{39,20\%} & \textbf{39,81\%} & \textbf{39,50\%} & \textbf{183\,040} \\
  \bottomrule
\end{tabularx}
...
\begin{hypothesebox}[Vérification sur Imagewoof]
  Sur Imagewoof, la Stratégie 2 atteint \textbf{39,20\,\%} sur Test HF — un résultat \textbf{nettement inférieur} aux baselines BL 2 (43,73\,\%) et BL 3 (47,19\,\%), et bien en deçà de la Stratégie 1 (53,68\,\%). Seule BL 1 (24,87\,\%) reste en dessous. Ce comportement diverge fortement de celui observé sur Animals-10 :
  \begin{itemize}
    \item \textbf{Comportement opposé à Animals-10} : sur Animals-10, la Stratégie 2 battait BL 1 et BL 2 de plus de $+10$ points HF. Sur Imagewoof, elle est devancée par BL 2 et BL 3 simples.
    \item \textbf{Robustesse inter-domaine néanmoins préservée} : malgré l'accuracy plus faible, l'écart HF/BF de la Stratégie 2 reste extrêmement resserré (39,20 vs 39,81), conforme à l'esprit de la méthode --- meilleur équilibrage HF/BF que toutes les autres approches sur ce dataset.
    \item \textbf{Hypothèse explicative} : sur Imagewoof, le co-training à ratio 16/48 par batch est mécaniquement dominé par les 48 échantillons BF, et le faible volume HF disponible (719 images train HF contre 1\,881 sur Animals-10) ne suffit pas à ce que le \textit{reweighting} HF spécialise efficacement les features fine-grained. La Stratégie 1, qui sépare strictement les phases BF puis HF, contourne ce problème.
  \end{itemize}
  Cette divergence est précieuse pour l'analyse : elle indique que les performances de la Stratégie 2 sont \textbf{sensibles au volume HF disponible} et au degré de granularité du problème. Une étude d'ablation du ratio HF/BF et des poids de loss serait nécessaire pour la rendre transposable sur des datasets fine-grained à faible volume.
\end{hypothesebox}
...
\section{Stratégie 3 -- Teacher-Student inspirée de Noisy Student}

% ---- 4.3.1 Protocole ----
\subsection{Protocole}

Cette stratégie adapte le schéma \textit{Teacher-Student} de Xie et al. (2020) à notre contexte multi-fidélité :

\begin{enumerate}
  \item \textbf{Teacher HF} : un ResNet-18 est entraîné uniquement sur HF (10 époques), puis figé en mode \texttt{eval()}.
  \item \textbf{Pseudo-labeling BF} : le Teacher produit des \textit{soft pseudo-labels} (probabilités Softmax) pour l'ensemble BF d'entraînement.
  \item \textbf{Student noised} : un nouveau ResNet-18 est entraîné \textit{from scratch} avec augmentation agressive (RandAugment, ColorJitter, RandomErasing, flips), sur un mélange :
  \begin{itemize}
    \item HF avec labels réels (\texttt{CrossEntropyLoss}),
    \item BF avec pseudo-labels mous (\textit{Soft Cross-Entropy}).
  \end{itemize}
\end{enumerate}

La perte globale est définie par :
\[
\mathcal{L}_{tot} = \mathcal{L}_{HF} + \alpha \cdot \mathcal{L}_{BF}, \quad \alpha=1.0
\]
...
\begin{tabularx}{\linewidth}{L{4.1cm} C{3cm} L{7.6cm}}
  ...
  \textbf{Paramètre} & \textbf{Valeur} & \textbf{Commentaire} \\
  \midrule
  Architecture Teacher/Student & ResNet-18 / ResNet-18 & Student avec dropout (p=0,3) sur la tête \\
  Entraînement Teacher & 10 époques HF & HF train = 1\,881 ; HF val = 471 \\
  Pseudo-labeling BF & Softmax (température 1,0) & 21\,213 images BF pseudo-labellisées \\
  Entraînement Student & 20 époques HF+BF & Loss mixte hard HF + soft BF \\
  Pondération BF & $\alpha = 1,0$ & Même poids global HF/BF dans la loss \\
  Augmentations Student & Agressives & RandAugment, ColorJitter, RandomErasing, flips \\
  \bottomrule
\end{tabularx}
...
Les résultats finaux de l'Étape 3 sont résumés ci-dessous.
Le critère prioritaire reste la précision sur \textbf{Test HF}.
...
\begin{tabularx}{\linewidth}{L{5.3cm} C{2.4cm} C{2.4cm} C{2.4cm} C{2.4cm}}
  ...
  \midrule
  \tablerowodd
  Stratégie 3 (Noisy Student adapté) & 52,72\% & 28,46\% & 40,59\% & 988\,560 \\
  \bottomrule
\end{tabularx}

\vspace{0.8em}
\begin{hypothesebox}[Lecture rapide des courbes d'entraînement]
  Le Teacher progresse en validation HF jusqu'à environ 45,65\% (époque 9), puis termine à 43,10\% (époque 10), indiquant une qualité de pseudo-labels limitée.
  Le Student réduit sa loss d'entraînement (4,46 $\rightarrow$ 3,13) et atteint 50,11\% en validation HF (époque 19), mais la généralisation finale reste insuffisante sur les trois tests.
\end{hypothesebox}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/strategy3_noisy_student_curves.png}
  \caption{Évolution des pertes Teacher/Student pour la Stratégie 3 (Noisy Student adapté au cadre multi-fidélité).}
  \label{fig:strategy3_noisy_student_curves}
\end{figure}

% ---- 4.3.3 Comparaison ----
\subsection{Comparaison avec les autres stratégies}
...
\begin{tabularx}{\linewidth}{L{4.5cm} C{2.2cm} C{2.2cm} C{2.2cm} C{2.5cm}}
  ...
  \tablerowodd
  Stratégie 2 (Co-training pondéré) & 70,12\% & 61,90\% & 66,01\% & 486\,720 \\
  \textbf{Stratégie 3 (Noisy Student adapté)} & \textbf{52,72\%} & \textbf{28,46\%} & \textbf{40,59\%} & \textbf{988\,560} \\
  \bottomrule
\end{tabularx}
...
\begin{itemize}
  \item \textbf{Objectif HF non atteint} : avec 52,72\% sur HF, la stratégie 3 est en dessous de BL 1 (56,20\%) et très loin des stratégies 1 et 2.
  \item \textbf{Effondrement BF} : la précision BF chute à 28,46\%, soit la plus faible de toutes les approches testées.
  \item \textbf{Coût défavorable} : le coût total (988\,560 CA) devient le plus élevé du panel, sans gain de performance.
  \item \textbf{Bilan global} : la stratégie 3 est dominée simultanément en précision et en coût par les stratégies précédentes.
\end{itemize}

% ---- 4.3.4 Analyse ----
\subsection{Analyse : pourquoi la stratégie 3 échoue dans ce contexte}

\begin{itemize}
  \item \textbf{Teacher insuffisamment performant} : un pseudo-labeling n'est utile que si le Teacher est déjà solide. Ici, sa validation HF reste modeste, ce qui injecte du bruit sémantique dans BF.
  \item \textbf{Accumulation de bruit} : BF est déjà un domaine dégradé ; lui ajouter des pseudo-labels imparfaits puis des augmentations agressives sur le Student peut amplifier l'erreur au lieu de la corriger.
  \item \textbf{Désalignement de domaine} : dans cette configuration, les pseudo-labels BF ne compensent pas le décalage HF/BF, et le Student n'apprend pas de représentations transférables vers le test HF.
  \item \textbf{Poids BF potentiellement trop élevé} : avec $\alpha=1.0$, la composante BF peut dominer l'optimisation malgré la priorité HF du projet.
\end{itemize}

\begin{decisionbox}
  \textbf{Conclusion Étape 3 (objectif HF prioritaire) :} l'adaptation directe de Noisy Student n'est pas validée dans notre cadre multi-fidélité actuel. Les performances chutent fortement alors que le coût augmente. Les prochaines variantes devront d'abord renforcer la qualité du Teacher et réduire l'influence des pseudo-labels BF (filtrage par confiance, pondération BF plus faible, augmentations moins agressives, ou Teacher plus robuste).
\end{decisionbox}

\subsection{Améliorations possibles de la méthode (Stratégie 3)}

Pour rendre cette approche exploitable, plusieurs paramètres critiques doivent être ré-optimisés :

\begin{itemize}
  \item \textbf{Qualité initiale du Teacher} : c'est le facteur principal. \textit{Comment optimiser :} entraîner plus longtemps, ajuster LR/scheduler, et sélectionner le meilleur checkpoint val HF avant pseudo-labeling.
  \item \textbf{Seuil de confiance pseudo-labels} : utiliser tous les pseudo-labels propage les erreurs. \textit{Comment optimiser :} ne conserver que les échantillons BF au-dessus d'un seuil de confiance (ex. max prob > 0,7/0,8).
  \item \textbf{Température de pseudo-labeling} : la température contrôle la netteté des distributions. \textit{Comment optimiser :} balayer $T \in [0,7, 2,0]$ et mesurer l'impact sur HF final.
  \item \textbf{Pondération BF ($\alpha$)} : \(\alpha=1\) peut sur-pondérer BF bruité. \textit{Comment optimiser :} tester des valeurs plus faibles (0,2 ; 0,4 ; 0,6 ; 0,8) avec priorité sur Test HF.
  \item \textbf{Intensité du bruit Student} : augmentation trop agressive peut détruire les signaux utiles. \textit{Comment optimiser :} réduire progressivement RandAugment/ColorJitter/RandomErasing et faire des ablations.
  \item \textbf{Mix HF/BF par batch} : un mélange non contrôlé peut défavoriser HF. \textit{Comment optimiser :} imposer un ratio HF minimal par batch (sampler dédié) pour ancrer l'optimisation sur les labels fiables.
  \item \textbf{Architecture Teacher/Student} : un Teacher plus capacitaire peut améliorer la qualité des pseudo-labels. \textit{Comment optimiser :} Teacher ResNet-34/EfficientNet-B0, Student plus léger ou équivalent, à protocole constant.
  \item \textbf{Fonction de perte BF} : la Soft CE simple peut être insuffisante. \textit{Comment optimiser :} tester KLDiv, entropie régularisée, ou pondération par confiance échantillon.
  \item \textbf{Curriculum d'apprentissage} : lancer d'emblée HF+BF pseudo peut être instable. \textit{Comment optimiser :} warm-up Student sur HF seul, puis injection progressive des données BF pseudo-labellisées.
\end{itemize}
